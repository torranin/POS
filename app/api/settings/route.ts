...
export async function PUT(request: Request) {
  try {
    ...
    const values: Record<string, string> = {
      ...
      currency: body.currency === "THB" ? "THB" : "THB",
      prices_include_vat: body.pricesIncludeVat ? "1" : "0",
      tax_invoice_enabled: body.taxInvoiceEnabled ? "1" : "0",
      tax_invoice_prefix: String(body.taxInvoicePrefix ?? "TAX").replace(/[^a-zA-Z0-9-]/g, "").slice(0, 12) || "TAX",
      default_payment_method: ["cash","qr","card"].includes(String(body.defaultPaymentMethod)) ? String(body.defaultPaymentMethod) : "cash",
      allow_discount: body.allowDiscount ? "1" : "0",
      max_discount_percent: String(Math.max(0, Math.min(100, Number(body.maxDiscountPercent) || 0))),
      rounding_mode: body.roundingMode === "whole" ? "whole" : "none",
